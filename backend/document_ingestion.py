...
def _ids_at_line_start(line: str) -> List[str]:
    ...
    if m:
        # Reject short numeric-only section numbers like 1.1, 2.1, 10.2
        # These are document section headings, not requirement IDs
        # Only accept if segments contain >= 3 digits (e.g. 001.002)
        seg = m.group(1).split(".")
        if all(len(x) <= 2 for x in seg):
            return []  # e.g. 1.1, 2.1, 10.2 — skip
        return [m.group(1)]
...
def _clean_module_name(raw: str) -> Optional[str]:
    ...
    # Keep only alphabetical characters, spaces, hyphens, and > separator
...
def parse_requirements_from_text(text: str) -> List[Dict]:
    ...
    for line in lines:
        ...
        # For ## heading lines: if the heading text contains a requirement ID,
        # strip the ## prefix and process as a normal requirement opener.
        # This handles SRS docs where each requirement is its own heading.
        if raw.startswith('##'):
            stripped_heading = re.sub(r'^#+\s*', '', raw).strip()
            heading_ids = _ids_at_line_start(stripped_heading)
            if not heading_ids:
                continue  # pure section heading, no req ID — skip as before
            raw = stripped_heading  # treat as a normal req line
...
def ingest_document(text: str, chunk_size_words: int = 1500) -> List[DocumentChunk]:
    ...
    if parsed:
        ...
        for i, req in enumerate(parsed):
            ...
            # Final safety: strip non-alpha chars but KEEP > (H3>H4 separator)
            doc_module = re.sub(r'[^A-Za-z\s>]', ' ', doc_module).strip()
            doc_module = re.sub(r'\s+', ' ', doc_module) or "General"
            # Normalise spacing around > separator
            doc_module = re.sub(r'\s*>\s*', ' > ', doc_module).strip()
